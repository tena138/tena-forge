    ...
    try {
      if (assignment.isTest) {
        ...
        if (mounted && confirmed) {
          context.push('/test/${assignment.id}');
        }
      } else {
        ...
        if (mounted) {
          ScaffoldMessenger.of(
            context,
          ).showSnackBar(const SnackBar(content: Text('완료 체크를 선생님에게 보냈습니다.')));
        }
      }
    } catch (_) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('완료 체크 가능 시간이나 학원 멤버십을 확인하세요.')),
        );
      }
    } finally {
  ...
  Widget build(BuildContext context) {
    ...
    if (assignment == null) {
      return const AppScaffold(
        title: '과제 상세',
        children: [PremiumCard(child: Text('과제를 찾을 수 없습니다.'))],
      );
    }
    return AppScaffold(
      title: assignment.title,
      ...
      children: [
        PremiumCard(
          ...
          child: Column(
            ...
            children: [
              Text(
                '제출 방식: ${assignment.submissionMode}',
                style: const TextStyle(fontWeight: FontWeight.w800),
              ),
              const SizedBox(height: 8),
              Text(
                '마감: ${assignment.dueAt?.toLocal().toString() ?? '없음'}',
              ...
              if (assignment.timeLimitMinutes != null) ...[
                const SizedBox(height: 8),
                Text(
                  '제한 시간: ${assignment.timeLimitMinutes}분',
                  style: const TextStyle(color: AppColors.muted),
                ),
              ],
            ],
