...
function BatchDetail({ batch }: { batch: Batch }) {
  ...
  return (
    <div className="grid gap-3 rounded-[8px] border border-white/10 bg-black/30 p-4 text-sm md:grid-cols-[1.1fr_1fr]">
      ...
      {isError ? (
        <div className="rounded-[8px] border border-red-400/20 bg-red-400/10 p-3 text-sm text-red-100">
          <div className="flex items-center gap-2 font-semibold">
            <AlertTriangle className="h-4 w-4" />
            처리 실패 원인
          </div>
          <div className="mt-3 space-y-2 text-xs leading-5">
            <p><span className="text-red-200/70">실패 단계:</span> {batch.failure_stage || "확인되지 않음"}</p>
            <p><span className="text-red-200/70">원인:</span> {errorReason(batch)}</p>
            <p><span className="text-red-200/70">대응:</span> {batch.failure_hint || "재처리하거나 원본 파일과 서버 설정을 확인하세요."}</p>
          </div>
        </div>
      ) : (
  ...
  return (
    <section>
      <div className="mb-3 flex items-center justify-between">
        ...
        <Link className="text-sm font-semibold text-violet-300 hover:text-violet-200" href="/batches">전체 보기</Link>
      </div>
