...
export default function CheckoutSuccessPage({ searchParams }: { searchParams: { paymentId?: string; type?: string } }) {
  const isAnnual = searchParams.type === "annual";
  return (
    <main className="flex min-h-screen items-center justify-center bg-[#f7f5f1] px-4 text-slate-950">
      <section className="w-full max-w-xl rounded-[28px] border border-slate-950/10 bg-white p-8 text-center shadow-[0_24px_90px_rgba(15,23,42,0.10)]">
        ...
        <p className="mt-3 text-sm leading-6 text-slate-600">
          {isAnnual ? "1년 이용권이 활성화되었습니다. 연간 결제는 자동 갱신되지 않습니다." : "선택한 월 구독 구성이 적용되었습니다. 대시보드에서 PDF 추출과 문제 DB 구성을 시작할 수 있습니다."}
        </p>
        {searchParams.paymentId && <p className="mt-4 rounded-[12px] bg-slate-50 px-4 py-3 text-xs font-bold text-slate-500">Payment ID: {searchParams.paymentId}</p>}
