...
class ArchiveFolderTests(unittest.TestCase):
    ...
    def test_folder_tree_and_batch_assignment_are_account_scoped(self):
        ...
        try:
            ...
            self.assertEqual([folder.name for folder in list_archive_folders(self.request, db)], ["수학", "미친개념 수2"])
            self.assertEqual(list_archive_folders(other_request, db), [])

            batch = Batch(name="0606 수2", problem_pdf_filename="problem.pdf", owner_id="owner-1")
            db.add(batch)
            db.commit()
            db.refresh(batch)
            ...
            self.assertEqual(db.query(ArchiveFolder).count(), 1)
        finally:
            db.close()
