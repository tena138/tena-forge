...
def main() -> None:
    if not REVIEW_EMAIL or not REVIEW_PASSWORD:
        print("Skipping PG review account bootstrap: PG_REVIEW_EMAIL or PG_REVIEW_PASSWORD is empty.")
        return

    validate_password_policy(REVIEW_PASSWORD, REVIEW_EMAIL, REVIEW_NAME)

    db = SessionLocal()
    try:
        now = datetime.utcnow()
        account = db.scalar(select(Academy).where(Academy.email == REVIEW_EMAIL))
        if not account:
            account = Academy(
                email=REVIEW_EMAIL,
                password_hash=hash_password(REVIEW_PASSWORD),
                academy_name=REVIEW_NAME,
            ...
            db.add(account)
            db.flush()
            print(f"Created PG review account: {REVIEW_EMAIL}")
        else:
            account.password_hash = hash_password(REVIEW_PASSWORD)
            account.academy_name = REVIEW_NAME
            ...
            print(f"Updated PG review account: {REVIEW_EMAIL}")
