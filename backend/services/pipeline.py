...
For visuals, visual_bbox is the authoritative visual asset: the system will crop the original page image by black/white ink pixels and preserve that crop. Return visual_schema only as a secondary editable aid when the visual is exactly reproducible from explicit mathematical facts, such as a simple coordinate-plane function graph with its equation/domain stated in the problem text, or a structured table with readable cells. Use cartesian_graph only for those exact analytic graphs, and structured_table only for visible tables or matrix-like grids. For geometry diagrams, scanned figures, piecewise graphs with open/closed points, dotted guide lines, printed textbook diagrams, hand-drawn diagrams, illustrations, photos, complex art, or any visual where fidelity matters, set visual_schema to null and rely on the visual_bbox crop. Do not redraw, approximate, or invent a diagram from memory. Define reusable graph expressions in math_model.expressions only when the expression is explicitly visible or stated, and reference them from graph objects with refs such as "expressions.f". Use plain graph expressions with x, +, -, *, /, ^ and common functions such as sin, cos, tan, sqrt, log, ln. Set visual_schema.source to "visual_and_problem_text" when explicit problem_text constraints shaped the schema, otherwise "visual_only". Do not invent graph equations, table values, labels, measurements, or constraints that are neither visible nor explicitly stated.
...
- visual_bbox is the authoritative visual asset and should tightly enclose the source image region that will be cropped by ink pixels. Return visual_schema and matching math_model only as a secondary aid when the visual is exactly reproducible from explicit mathematical facts, such as a simple coordinate-plane function graph with its equation/domain stated in the problem text, or a structured table with readable cells; include confidence and set it to at least 0.85. Use cartesian_graph only for those exact analytic graphs, and structured_table only for visible tables or matrix-like grids. For geometry diagrams, scanned figures, piecewise graphs with open/closed points, dotted guide lines, printed textbook diagrams, hand-drawn diagrams, illustrations, photos, complex art, or anything requiring visual fidelity, set visual_schema to null and rely on the visual_bbox crop. Do not redraw, approximate, or invent diagrams, graph equations, table values, labels, measurements, or constraints that are neither visible nor explicitly stated.
    ...
    with ThreadPoolExecutor(max_workers=_ai_worker_count(len(candidates), len(model_pool))) as executor:
        ...
        for future, payload in _completed_futures_with_heartbeat(
            ...
            try:
                ...
                if problem.get("has_visual") and not problem.get("visual_url"):
                    page = page_by_index.get(int(problem.get("page_index") or 0))
                    if page:
                        _attach_visual_from_page_bbox(problem, page, batch_id, mark_review=True)
            except Exception:
